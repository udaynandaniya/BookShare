...
import { Edit, Trash2, MapPin, Eye, Heart, Share2 } from "lucide-react"
...
export function BookEditCard({ book, onEdit, onDelete, onView }: BookEditCardProps) {
  ...
  const handleShare = async () => {
    ...
    if (navigator.share) {
      ...
      MySwal.fire({
        title: "Link Copied!",
        text: "Book link copied to clipboard",
        icon: "success",
        background: theme === "dark" ? "#1e1b4b" : "#ffffff",
        color: theme === "dark" ? "#ffffff" : "#1f2937",
        timer: 1500,
        ...
        <div className="relative aspect-[4/5] overflow-hidden">
          ...
          <div className="absolute top-3 right-3 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-x-2 group-hover:translate-x-0">
            ...
            <Button
              ...
              className={`${
                theme === "dark"
                  ? "bg-black/70 hover:bg-red-600/80 border-red-400/30"
                  : "bg-white/80 hover:bg-red-500/80 border-red-300/30"
              } p-2 h-9 w-9 rounded-full backdrop-blur-md shadow-lg transition-all duration-300 hover:scale-110 border`}
              onClick={(e) => {
                e.stopPropagation()
                setIsLiked(!isLiked)
